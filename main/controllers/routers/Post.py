...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(get_current_user), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    posts = db.query(post.Post, func.count(post.Vote.post_id).label("votes")).join(
        post.Vote, post.Vote.post_id == post.Post.id, isouter=True).group_by(post.Post.id).filter(post.Post.title.contains(search)).limit(limit).offset(skip).all()
    return posts


@router.post("/", status_code=status.HTTP_201_CREATED, response_model=postschemas.Post)
def create_posts(Post: postschemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
...
def update_post(id: int, updated_post: postschemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
